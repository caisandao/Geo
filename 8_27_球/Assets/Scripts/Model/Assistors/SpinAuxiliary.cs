...
public class SpinAuxiliaryTool : AuxiliaryTool
{
    ...
    public override Auxiliary GenerateAuxiliary(Geometry geometry, FormInput formInput)
    {
        ...
        if (vertexUnits.Length != 3 && vertexUnits.Length != 4 && vertexUnits.Length!=9)
            return null;
    ...
    public void GenerateResolvedBody(Geometry geometry)
    {
        ...
        else if (vertexUnits.Length == 3)
        {
            ...
            GeoCircular circular = new GeoCircular(new VertexUnit[] { vertex1, vertex2, vertex3 }, radius, radius, CircularType.Spheree);
            bool showFace = false;
            if (circular.Circular().IsNormalCircular()) 
            {
                showFace = true;
        ...
        //Spheree
        else if (vertexUnits.Length ==9)
        {
            VertexUnit vertex1 = vertexUnits[0];
            VertexUnit vertex2 = vertexUnits[1];
            VertexUnit vertex3 = vertexUnits[2];
            VertexUnit vertex4 = vertexUnits[3];
            VertexUnit vertex5 = vertexUnits[4];
            VertexUnit vertex6 = vertexUnits[5];
            VertexUnit vertex7 = vertexUnits[6];
            VertexUnit vertex8 = vertexUnits[7];
            VertexUnit vertex9 = vertexUnits[8];
            float radius1 = vertexUnits[3].Position().z;
            float radius2 = vertexUnits[2].Position().z;
            GeoCircular circular = new GeoCircular(new VertexUnit[] { vertex1, vertex2, vertex3, vertex4, vertex5, vertex6, vertex7, vertex8, vertex9 }, radius1, radius2, CircularType.Spheree);
            bool showFace = false;
            if (circular.Circular().IsNormalCircular())
            {
                showFace = true;
